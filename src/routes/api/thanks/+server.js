...
const thanksFilePath = path.join(__dirname, '../../../../static/data/thanks.json');
...
export async function GET() {
    try {
        const data = JSON.parse(fs.readFileSync(thanksFilePath, 'utf8'));
        return json(data);
    } catch (error) {
...
export async function POST() {
    try {
        ...
        return json({ count: 0, error: 'Failed to update thanks count' }, { status: 500 });
    }
}
